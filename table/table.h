...
#ifndef TABLE_VALUE_TYPE
#define TABLE_VALUE_TYPE double
#endif
...
bool table_get(Table *table, const char *key, TValue *value);
...
static bool adjust_capacity(Table *table, size_t capacity)
{
    ...
    Key *keys = (Key *)malloc(sizeof(Key) * capacity);
    if (keys == NULL) {
        free(entries);
        return false;
    }
...
{
    key->chars = (char *)malloc(sizeof(char) * (len + 1));
...
bool table_insert(Table *table, const char *key_chars, TValue value)
{
...
bool table_get(Table *table, const char *key_chars, TValue *value)
{
